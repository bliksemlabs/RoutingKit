...
int main() {
    const string endpoint = "tcp://*:4245";
    ...
    std::string sql_node = "SELECT id, ST_X(ST_Transform(geom, 4326)), ST_Y(ST_Transform(geom, 4326)) FROM (SELECT jte_id_beg AS id, ST_StartPoint(geom) AS geom FROM wegvakken JOIN (SELECT min(gid) as gid, jte_id_beg FROM wegvakken GROUP BY jte_id_beg) AS b1 USING (jte_id_beg, gid) UNION ALL SELECT jte_id_end AS id, ST_EndPoint(geom) AS geom FROM wegvakken JOIN (SELECT jte_id_end, MIN(gid) AS gid FROM wegvakken JOIN (SELECT DISTINCT jte_id_end FROM wegvakken EXCEPT SELECT jte_id_beg FROM wegvakken) AS e1 USING (jte_id_end) GROUP BY jte_id_end) AS e2 USING (jte_id_end, gid)) AS t3 ORDER BY id;";

    std::string sql_edge = "SELECT wvk_id, jte_id_beg, jte_id_end, ST_Transform(geom, 4326), ST_Length(geom)::int4 AS distance FROM wegvakken;";
