...
json geojson_feature_polygon_from_envelope(float (&rect_min)[2], float (&rect_max)[2], std::string id, std::string orig_id) {
    return {
        { "id", id },
        { "properties",
            {
...
json geojson_feature_point(float longitude, float latitude, std::string id, std::string orig_id) {
    return {
        { "id", id },
        { "properties",
            {
...
void GeoPositionToNodeRTree::expand_search_rect(float &query_latitude, float &query_longitude, float (&search_rect_min)[2], float (&search_rect_max)[2], float &radius, std::vector<unsigned> &results) {
    // std::cout << query_latitude << "," << query_longitude << std::endl;    
    // std::cout << geojson_feature_point(query_longitude, query_latitude, "query", "query").dump(1) << ",";
	...
	while (nhits == 0) {
        ...
        // std::cout << "expand_search_rect " << radius << nhits << std::endl;
	    // std::cout << "expand_search_rect " << search_rect_min[0] << ", " << search_rect_min[1] << " " << search_rect_max[0] << ", " << search_rect_max[1] << std::endl;

        // std::cout << geojson_feature_polygon_from_envelope(search_rect_min, search_rect_max, "expand_search_rect", "expand_search_rect").dump(1) << "," << std::endl;
    }
}

void GeoPositionToNodeRTree::shrink_optimal_rect(float &query_latitude, float &query_longitude, float (&optimal_rect_min)[2], float (&optimal_rect_max)[2], float &radius) {
	// std::cout << query_latitude << "," << query_longitude << std::endl;
	...
	// std::cout << "shrink_optimal_rect " << optimal_rect_min[0] << ", " << optimal_rect_min[1] << " " << optimal_rect_max[0] << ", " << optimal_rect_max[1] << std::endl;

    // std::cout << geojson_feature_polygon_from_envelope(optimal_rect_min, optimal_rect_max, "expand_optimal_rect", "shrink_optimal_rect").dump(1) << "," << std::endl;
}


bool GeoPositionToNodeRTree::within_optimal_rect(float (&optimal_rect_min)[2], float (&optimal_rect_max)[2], std::vector<unsigned> &results, unsigned &min_diff_id) {
	float min_diff = inf_weight;
	for (unsigned i : results) {
		if (optimal_rect_min[0] <= imin[i][0] && optimal_rect_min[1] <= imin[i][1] && optimal_rect_max[0] >= imax[i][0] && optimal_rect_max[1] >= imax[i][1]) {
			min_diff_id = invalid_id;
			return true;
		} else {
			float overlap_x = fmax(0, fmin(optimal_rect_max[0], imax[i][0]) - fmax(optimal_rect_min[0], imin[i][0]));
			float overlap_y = fmax(0, fmin(optimal_rect_max[1], imax[i][1]) - fmax(optimal_rect_min[1], imin[i][1]));
            ...
            // std::cout << "within_optimal_rect " << imin[i][0] << ", " << imin[i][1] << " " << imax[i][0] << ", " << imax[i][1] << std::endl;

			if (diff < min_diff) {
				min_diff_id = i;
				min_diff = diff;

                float min_convert[2] = { imin[i][0], imin[i][1] };
                float max_convert[2] = { imax[i][0], imax[i][1] };

                // std::cout << geojson_feature_polygon_from_envelope(min_convert, max_convert, std::to_string(i), "within_optimal_rect").dump() << "," << std::endl;
                // std::cout << "within_optimal_rect " << min_diff_id << " (" << min_diff << ")" << std::endl;
			}
		}
	}
...
void GeoPositionToNodeRTree::expand_contained_rect(float (&optimal_rect_min)[2], float (&optimal_rect_max)[2], unsigned &min_diff_id, float (&search_rect_min)[2], float (&search_rect_max)[2]) {
    ...
    // std::cout << geojson_feature_polygon_from_envelope(search_rect_min, search_rect_max, "expand_search_rect", "expand_contained_rect").dump(1) << "," << std::endl;
	// std::cout << "expand_contained_rect " << search_rect_min[0] << ", " << search_rect_min[1] << " " << search_rect_max[0] << ", " << search_rect_max[1] << std::endl;
}

bool GeoPositionToNodeRTree::validate_search_rect(float (&search_rect_min)[2], float (&search_rect_max)[2], std::vector<unsigned> &results) {
	int nhits = tree.Search(search_rect_min, search_rect_max, MySearchCallback, (void *) &results);
...
std::vector<GeoPositionToNodeRTree::RTreeQueryResult> GeoPositionToNodeRTree::find_all(float query_latitude, float query_longitude) {
   
    std::vector<GeoPositionToNodeRTree::RTreeQueryResult> ret;
    ...
    expand_search_rect(query_latitude, query_longitude, search_rect_min, search_rect_max, radius, results);
	...
	shrink_optimal_rect(query_latitude, query_longitude, optimal_rect_min, optimal_rect_max, radius);
	...
	unsigned min_diff_id = invalid_id;
	bool has_contained = within_optimal_rect(optimal_rect_min, optimal_rect_max, results, min_diff_id);

	/* we need to search a bit broader to have contained results */
	if (!has_contained) {
		expand_contained_rect(optimal_rect_min, optimal_rect_max, min_diff_id, search_rect_min, search_rect_max);
		has_contained = validate_search_rect(search_rect_min, search_rect_max, results);
	}
